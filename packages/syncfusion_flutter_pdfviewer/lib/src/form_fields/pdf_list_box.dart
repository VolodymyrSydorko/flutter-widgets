...
class PdfListBoxFormFieldHelper extends PdfFormFieldHelper {
  ...
  PdfListBoxFormFieldHelper(
    this.pdfListBoxField,
    int pageIndex, {
    this.onValueChanged,
  ...
  PdfListBoxFormField getFormField() {
    ...
    final List<String> selectedItems = List<String>.from(
      pdfListBoxField.selectedValues,
      growable: false,
    );
  ...
  void invokeValueChanged(List<String>? newValue) {
    if (!listEquals(listBoxFormField._selectedItems, newValue)) {
      if (newValue == null || newValue.isEmpty) {
        if (!isMultiSelect) {
          newValue = List<String>.unmodifiable(<String>[
            listBoxFormField.items[0],
          ]);
        } else {
      ...
      if (onValueChanged != null) {
        onValueChanged!(
          PdfFormFieldValueChangedDetails(listBoxFormField, oldValue, newValue),
        );
      }
        ...
        child: Container(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              final bool selected = widget.selectedItems.contains(
                widget.items[index],
              );
              return Container(
...
void _showListBoxDialog(
  ...
  PdfListBoxFormFieldHelper listBoxHelper,
) {
  final bool isMaterial3 = Theme.of(context).useMaterial3;
  List<String> newItems = List<String>.from(
    listBoxHelper.listBoxFormField._selectedItems!,
  );
  showDialog(
